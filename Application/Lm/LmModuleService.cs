        ...
        */
        ...
        public async Task<bool> EnsureInstalledAndInitialized(string token, string expectedInn)
        {
            ...
            if (!statusReady)
            {
                ...
                {
                    SetProgress(60, "ЛМ ЧЗ: переустановка из-за недоступного API");
                    _log.LogUser("⚠️ ЛМ установлен по GUID, но API не поднялся после запуска службы. Запускаем переустановку ЛМ ЧЗ.");
                    await _installer.ReinstallExisting("API не поднялся после запуска службы Regime");
                    return await StartApiAndInitialize(token, "после переустановки из-за недоступного API");
                }
            }
