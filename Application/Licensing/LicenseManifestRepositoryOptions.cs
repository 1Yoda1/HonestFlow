    ...
    public sealed class LicenseManifestRepositoryOptions
    {
        ...
        public void Validate()
        {
            if (ManifestUrl == null || !ManifestUrl.IsAbsoluteUri)
                throw new ArgumentException("An absolute grant.json URL is required.", nameof(ManifestUrl));

            if (ManifestUrl.Scheme != Uri.UriSchemeHttp && ManifestUrl.Scheme != Uri.UriSchemeHttps)
                throw new ArgumentException("The grant.json URL must use HTTP or HTTPS.", nameof(ManifestUrl));

            if (SignatureUrl == null || !SignatureUrl.IsAbsoluteUri)
                throw new ArgumentException("An absolute grant.json.sig URL is required.", nameof(SignatureUrl));

            if (SignatureUrl.Scheme != Uri.UriSchemeHttp && SignatureUrl.Scheme != Uri.UriSchemeHttps)
                throw new ArgumentException("The grant.json.sig URL must use HTTP or HTTPS.", nameof(SignatureUrl));
