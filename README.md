...
| Параметр | Значение |
|---|---|
| Приложение | C# / WinForms / `net6.0-windows` / `win-x64` |
...
1. Получите опубликованный `HonestFlow.exe` и эксплуатационную конфигурацию у ответственного за развёртывание.
...
> [!NOTE]
> У HonestFlow нет мастера установки или собственного MSI. Публикуется один EXE, зависящий от установленного .NET 6 Desktop Runtime. Каталог приложения одновременно является каталогом локальной конфигурации и резервных дистрибутивов.
...
Текущий технический приоритет — переход с `net6.0-windows` на поддерживаемую версию .NET с проверкой совместимости на рабочих точках. Следующие задачи фиксируются в issue tracker; этот раздел не задаёт сроков выпуска.
