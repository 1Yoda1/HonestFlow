...
| Эксплуатация | Локальная и удалённая конфигурация, кэш дистрибутивов, обязательное самообновление |
...
|---|---|
| Состояние рабочего места приходится определять по нескольким службам, портам и API | Формируется единый снимок состояния ЛМ ЧЗ, контроллера, ЕСМ, ККТ и удалённого доступа |
| Повторный запуск полного комплекта установщиков создаёт лишний риск | Сначала строится покомпонентный план; установка выполняется только для отсутствующих, устаревших или явно выбранных компонентов |
| Дистрибутивы и версии расходятся между точками | Версии задаются централизованно, допускаются переопределения для конкретного клиента, файлы загружаются в общий кэш |
| Сбой ЛМ ЧЗ не всегда требует полной переустановки | Разделены сценарии инициализации, восстановления служб, переустановки и восстановления базы |
| Для обращения в поддержку данные собираются вручную и непоследовательно | Приложение формирует структурированный ZIP-архив из известных журналов и системного снимка |
| Удалённая помощь зависит от наличия и исправности агента доступа | HonestFlow проверяет RuDesktop, устанавливает проверенный пакет и хранит локальное состояние настройки |
| Устаревшая версия инструмента продолжает использоваться на точке | При запуске проверяется опубликованная версия; обновление применяется с резервной копией и попыткой отката |
| Доступ к административным операциям должен быть ограничен | Решение принимается по подписанному манифесту, клиенту, идентификатору устройства, версии приложения и набору разрешений |
...
- подготовка .NET 10 Desktop Runtime, необходимого обслуживаемым компонентам.
...
- встроенный реестр доверенных открытых ключей без размещения закрытого ключа в приложении;
...
Решение разделено на пользовательский интерфейс, прикладные сценарии, инфраструктурные адаптеры и модели данных. Зависимости прикладного слоя от внешней среды вынесены в интерфейсы там, где это требуется для тестирования и замены реализаций.

| Путь | Ответственность |
|---|---|
| `Program.cs` | Composition root: запуск UI, логирование, обновление, идентификатор устройства, конфигурация и лицензирование |
...
| `Application/Bootstrap/` | Выбор удалённого или локального режима и сборка runtime-зависимостей |
| `Application/Core/` | Абстракции прогресса и прикладного логирования |
| `Application/DeviceIdentity/` | Контракт стабильного локального идентификатора устройства |
...
| `Application/Installation/` | Проверка версий, построение и исполнение плана установки |
...
| `Infrastructure/Process/` | Запуск внешних процессов с захватом результата |
| `Infrastructure/Updates/` | Обнаружение и применение самообновления |
| `Infrastructure/WindowsServices/` | Обнаружение и управление службами Windows |
...
| `HonestFlow.LicenseSigning/` | Библиотека формирования ECDSA-подписи манифеста; не входит в runtime-код приложения |
| `HonestFlow.Tests/` | xUnit-тесты лицензирования, кэша, идентификатора устройства, статусов ЕСМ, RuDesktop и prerequisites |
...
1. Получите опубликованный `HonestFlow.exe` и эксплуатационную конфигурацию у сопровождающей стороны.
2. Разместите EXE в отдельном каталоге, доступном для обновления.
3. При локальном режиме разместите разрешённые конфигурационные файлы рядом с EXE и дистрибутивы в `Distr`.
4. Запустите `HonestFlow.exe`; Windows запросит повышение прав.

> [!NOTE]
> Отдельного MSI для HonestFlow в репозитории нет. Публикуется single-file, framework-dependent EXE; требуемый .NET 6 Desktop Runtime должен быть установлен заранее.

### Для разработчика

Необходимы Windows, .NET SDK 6.0 и Git. Visual Studio может использовать файл решения `HonestFlow.slnx`, но для сборки достаточно .NET CLI.
...
Production-конфигурация и закрытые ключи для локальной разработки не требуются для компиляции и большинства модульных тестов.
...
Удалённые дистрибутивы сохраняются в `%ProgramData%\HonestFlow\cache\installers`. Существующие legacy-кэши регистрируются как дополнительные источники чтения без копирования их содержимого.

> [!WARNING]
> Суффикс `_encrypted` исторический: эти JSON-файлы используют XOR `0xAA` и Base64 для совместимости, а не криптографическое шифрование. Их следует считать чувствительными данными в открытом виде.

## Диагностика

Архив создаётся в `%ProgramData%\HonestFlow\diagnostics` и содержит только найденные источники. Для журналов ЛМ ЧЗ, ЕСМ и ККТ выбираются данные за последние два дня; собственные журналы HonestFlow добавляются из централизованного каталога.

| Группа | Собираемые данные |
|---|---|
| HonestFlow | Файлы `*.log` текущего приложения |
...
| Windows | версия ОС и .NET, архитектура, пользователь и компьютер, диски, сетевые интерфейсы, процессы, службы и результаты `sc query` для известных служб |
| Контекст | версия HonestFlow, рабочие каталоги, адрес точки и доступные идентификаторы удалённого доступа |
...
HonestFlow использует JSON-манифест `licenses.json` и отделённую подпись `licenses.json.sig`. Подпись создаётся вне приложения закрытым ECDSA P-256 ключом и проверяется встроенным открытым ключом с SHA-256. Изменение любого байта манифеста после подписания делает подпись недействительной.

Решение о доступе учитывает:

- валидность структуры, ревизию и срок действия манифеста;
...
- источник манифеста и допустимый `OfflineGraceHours` при работе из кэша.

Сначала читается указатель `/licenses/licenses-current.json`, затем соответствующая версия манифеста и подписи. Успешно проверенный снимок атомарно сохраняется в `%ProgramData%\HonestFlow\license-cache`; его метаданные защищаются Windows DPAPI в области `LocalMachine`. Повреждённый, неподписанный или устаревший снимок не становится доверенным только потому, что находится локально.
...
Не добавляйте в Git, issues, Pull Request, release assets или скриншоты:

- `ips_encrypted.json`, `support_mail_encrypted.json`;
- `yandex_public_key.txt`, `yandex_public_url.txt`;
- `licenses.json`, `licenses.json.sig`;
- закрытые ключи и сертификаты: `*.pem`, `*.key`, `*.pfx`, `*.p12`;
- кэши установщиков, runtime-пакеты и локальное состояние `%ProgramData%\HonestFlow`;
...
- создаёт неизменяемую по имени копию в `artifacts\HonestFlow\versions\<Version>`;
...
> [!CAUTION]
> Текущий target framework приложения — `net6.0-windows`. .NET 6 снят с поддержки; это принятое ограничение текущей кодовой базы, которое необходимо учитывать перед широким распространением сборки.
...
Ближайшие инженерные приоритеты текущей кодовой базы:

- переход с `net6.0-windows` на поддерживаемую версию .NET после проверки совместимости на рабочих точках;
- расширение автоматизированных сценариев для установки, самообновления и диагностического архива;
- дальнейшее уменьшение объёма чувствительных данных в журналах и пакетах диагностики;
- формализация воспроизводимой публикации лицензий и release assets.

Roadmap не является обязательством по срокам. Изменения принимаются после проверки совместимости с эксплуатационной средой.
...
3. Не добавляйте production-конфигурацию, реальные идентификаторы клиентов, SMTP-реквизиты, лицензии, ключи, журналы или диагностические архивы.
...
Проект разработан для практического инженерного сопровождения рабочих мест и находится в активной эксплуатации и развитии.
