...
Текущий production startup запускает `HonestFlow Free`: публичная доверенная проверка обновления → локальная инициализация → основное WPF-окно с диагностикой этого компьютера. Для запуска не требуются вход, ClientId, DeviceId, серверная конфигурация или лицензия. Сохранённое окно авторизации временно остаётся основой будущего явного сценария «Подключить Service» и автоматически не открывается.
...
1. запускает локальную диагностику Free без обязательного серверного входа;
...
Production API traffic always uses `https://api.honestflow.ru/`. Текущий Free startup обращается только к публичному trusted SelfUpdate endpoint и не создаёт auth/session/device state. Сохранённый Service-контур после авторизации получает версии и описания установочных файлов из `GET /api/configuration/current`; файл кэшируется или запускается только после проверки server-provided size и SHA-256. Legacy Yandex-код остаётся только для недостижимой compatibility-ветки и не является источником normal production startup/install/update. Параметры SMTP, напротив, сначала читаются локально и только затем удалённо.
